...
import { generateSalesPitch } from "@/lib/openaiClient";
...
export async function POST(request: Request) {
  ...
  try {
    const pitch = await generateSalesPitch(shop);
    return NextResponse.json({ pitch });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: describeFetchError(error, "営業文の生成") }, { status: 500 });
  }
}
