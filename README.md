...
-地域・業種から店舗検索（どちらか一方のみの入力にも対応、都道府県・駅名周辺の指定も可能）
-Google Places API連携
-ホームページ未設定の可能性がある店舗を抽出
-店舗情報表示（店舗名・住所・電話番号・営業時間・評価）
-Googleマップリンク表示
-AIによるホームページたたき台生成（キャッチコピー〜お問い合わせまで7セクション）
-ホームページ風プレビュー表示（お問い合わせの電話番号はタップして発信可能）
-AIによる営業提案文の下書き生成
-生成したHPプレビューと店舗情報（住所・電話番号・営業時間・評価・Googleマップ等）を単体のHTMLファイルとしてダウンロード
...
- Google Places API（Text Search / Nearby Search）は1回の検索につき最大20件までしか取得できず、ページングは未実装のため、実質的な検索対象は上位20件までです（画面表示件数の上限は設けていません）。
- 地域・業種のどちらか一方だけを入力した場合、もう一方は「店舗全般」「全国」を対象として扱われます。ただし業種を指定しない場合、Googleの検索結果は知名度の高いチェーン店が上位に来やすく、そうした店舗は概ね公式サイトを持つため、該当件数が少なくなる（0件になる）傾向があります。
